# TLEするが解法としては解説通り
def bfs(num, start = 0 ,graph ,o_graph ,cost, limit)
  # 始点からの距離
  # -1は未探索を表す
  dist = Array.new(num+1) {-1}
  # キュー
  queue = []
  # 始点の情報を追加
  dist[start] = 0
  ...
  while (!queue.empty?)
    ...
    graph[t].each do |it|
      ...
      # 直前のノードの距離+1
      dist[it] = dist[t] + 1
      next if dist[it] > limit # タクシーが届く範囲外は探索不要
      o_graph[start] << [it, cost] # タクシーで降りられる場所すべてをグラフに登録
      queue.push(it)
    end
  end
end
...
# タクシーの運賃C[i]円、タクシーiは最大R[i]本の道路しか通れない
C,R = N.times.map { gets.split.map(&:to_i) }.transpose
# 町Aと町Bが繋がっている
# 町と道路の関係を示したグラフ
graph_bfs = Array.new(N+1) { Array.new }
K.times do
...
# ダイクストラ法の為のグラフ(タクシー乗車時を示す)
graph_dik = Array.new(N+1) { Array.new }
N.times do |i|
  bfs(N, i+1, graph_bfs, graph_dik, C[i], R[i])
end

dist = dijkstra(1, graph_dik, N)
puts dist[N]